...
os.makedirs((cwd / Path("Processed Documents")), exist_ok = True)
...
#for files and folders (entries) in the cwd
    if entry.is_dir():
        ...
        #list_dir.sort()
        #this is here in case we can't figure out why the sorting doesn't work

        pdf_ims = []
        #create an empty list to store the pdfs of the individual pages

        if str(cwd / Path("Processed Documents")) in subfolder or str(cwd /Path("__pycache__")) in subfolder or str(cwd /Path(".git")) in subfolder:
